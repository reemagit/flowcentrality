...
def main(args):
    ...
    gse = GEOparse.get_GEO(geo='GSE8581', destdir=join(args.out_expr_dir,'raw'))
    ...
    disease_cls = [gse.gsms[gsm].metadata['title'][0] for gsm in gse.gsms if 'COPD' in gse.gsms[gsm].metadata['title'][0] and 'CaseX' not in gse.gsms[gsm].metadata['title'][0]]
    healthy_cls = [gse.gsms[gsm].metadata['title'][0] for gsm in gse.gsms if 'Control' in gse.gsms[gsm].metadata['title'][0]]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process GSE8581')
